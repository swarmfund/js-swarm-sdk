 ...
 * Documents.
 */
class Documents extends _resource_group_base.ResourceGroupBase {
  /**
   * Document types.
   */
  get types() {
  ...
  async create(documentType, mimeType) {
    let response = await this._makeCallBuilder().post({
      data: {
        type: documentType,
    ...
    let formData = (0, _lodash.omit)(response.data, ['id', 'url', 'resourceType']);
