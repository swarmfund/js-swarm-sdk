 ...
 * TFA factors.
 */
class Factors extends _resource_group_base.ResourceGroupBase {
  /**
   * Get all TFA factors.
   *
   ...
   */
  createTotpFactor() {
    return this._makeCallBuilderWithSignature().post({ data: { type: 'totp' } });
  }
  ...
  changePriority(factorId, priority) {
    return this._makeCallBuilderWithSignature().appendUrlSegment(factorId).patch({ data: { attributes: { priority } } });
  }
  ...
  async verifyPasswordFactorAndRetry(tfaError, password) {
    ...
    let factorWallet = _wallet.Wallet.fromEncrypted(meta.keychainData, kdfParams, meta.salt, email, password);

    let otp = factorWallet.keypair.sign(meta.token).toString('base64');

    await this._makeCallBuilder(meta.walletId).appendUrlSegment([meta.factorId, 'verification']).put({
      data: {
        attributes: {
          token: meta.token,
          otp
  ...
  async verifyTotpFactorAndRetry(tfaError, otp) {
    await this._makeCallBuilder(tfaError.meta.walletId).appendUrlSegment([tfaError.meta.factorId, 'verification']).put({
      data: {
        attributes: {
