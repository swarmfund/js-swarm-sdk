      ...
      if (error.response && error.response.status) {
        ...
        if (horizonError) {
          ...
          switch (error.response.status) {
            case 404:
              error = new horizonErrors.NotFoundError(error, _this._axios);
              break;

            case 500:
              error = new horizonErrors.InternalServerError(error, _this._axios);
              break;

            default:
              error = new horizonErrors.HorizonError(error, _this._axios);
          }
        } else {
          switch (error.response.status) {
            case 400:
            ...
            default:
              error = new errors.ApiError(error, _this._axios);
          }
        }
      }
