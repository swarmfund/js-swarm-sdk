...
class Wallet {
  ...
  static fromEncrypted(keychainData, kdfParams, salt, email, password) {
    ...
    return new Wallet(email, _base.Keypair.fromSecret(decryptedKeychain.seed), decryptedKeychain.accountId, _sjclSwarm2.default.codec.hex.fromBits(rawWalletId));
  }
  ...
  static deriveId(email, password, kdfParams, salt) {
    ...
    return _sjclSwarm2.default.codec.hex.fromBits(walletId);
  }
  ...
  encrypt(kdfParams, password) {
    ...
    this._id = _sjclSwarm2.default.codec.hex.fromBits(rawWalletId);
